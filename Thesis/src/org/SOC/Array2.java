					...
					if ((a1[xpos][ypos + 1].isVisited == false) && success && this.backtrack(xpos, ypos + 1, destx, desty, list2)) {
						...
						if (destx < xpos) { // For minimal routing
							...
							if (success) {
								...
								// Instead of substracing two rectangles, I just slice them.
								
								temp = unreached;
								temp[0] = xpos - 1; 
								
								// Rectangle after slice
										
								System.out.println("temp array: " + temp[0] + ","+ temp[1] + ","+ temp[2] + ","+ temp[3]);
								unionArray(unreached, temp);
...
//					if (node.east_link_is_OK.getRT() != null) {
	...
	public boolean listCheck(int[][] arr1, int[] arr2) {
		for (int[] rec : arr1) {
			if (rec == arr2) {
				return false;
			}
