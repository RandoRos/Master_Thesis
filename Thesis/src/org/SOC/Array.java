...
public class Array {
	...
	public Array(int integer, int integer2) {
		this.xsize = integer;
		this.ysize = integer2;
	...
	public boolean backtrack(int xpos, int ypos, int destx, int desty, ArrayList<String> list2) {
		...
		if (ypos <= desty) {
			if (desty == ypos && destx < xpos) {
				...
				if ((a1[xpos - 1][ypos].isVisited == false) && success && this.backtrack(xpos - 1, ypos, destx, desty, list2)) {
					return true;
				} else {
//					if (list2.size() != 0) {
//						System.out.println("Accessing List2 Node: " + "[" + ypos + "][" + xpos + "]");
//						//node.north_link_is_OK.addRT(compList(list2));
//						if (list2.size() > 1) {
//							list2.remove(0);
//						}
//					}
					System.out.println("Accessing List2 Node: " + "[" + xpos + "][" + ypos + "]");
					if (!(a1[xpos - 1][ypos].north_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
						for (String s1 : a1[ypos - 1][xpos].north_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					if (!(a1[xpos - 1][ypos].east_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
						for (String s1 : a1[ypos - 1][xpos].east_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					if (!(a1[xpos - 1][ypos].south_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
						for (String s1 : a1[ypos - 1][xpos].south_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					
					for (String rec : list2) {
						node.north_link_is_OK.addRT(rec);
					}
				...
				if (!(success)) {
					unreached = xpos + "," + (ypos + 1) + "," + xpos + "," + (a1[xpos].length - 1);
...
//					if (list2.size() != 0) {
//						System.out.println("Accessing List2 Node: " + "[" + ypos + "][" + xpos + "]");
//						//node.east_link_is_OK.addRT(compList(list2));
//						if (list2.size() > 1) {
//							list2.remove(0);
//						}
//					}
					System.out.println("Accessing List2 Node: " + "[" + xpos + "][" + ypos + "]");
					if (!(a1[xpos][ypos + 1].north_link_is_OK.getRT().isEmpty()) && (a1[xpos][ypos + 1].isVisited == true)) {
						for (String s1 : a1[xpos][ypos + 1].north_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					if (!(a1[xpos][ypos + 1].east_link_is_OK.getRT().isEmpty()) && (a1[xpos][ypos + 1].isVisited == true)) {
						for (String s1 : a1[xpos][ypos + 1].east_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					if (!(a1[xpos][ypos + 1].south_link_is_OK.getRT().isEmpty()) && (a1[xpos][ypos + 1].isVisited == true)) {
						for (String s1 : a1[xpos][ypos + 1].south_link_is_OK.getRT()) {
							list2.add(compList(list2, s1));
						}
					}
					
					for (String rec : list2) {
						node.east_link_is_OK.addRT(rec);
					}
					list2.removeAll(list2);
				}
			} else {
				if (ypos != this.ysize - 1) {
					...
					success = node.getEast_link();
					if (!(success)) {
						//if (!(node.east_link_is_OK.checkRT("east\t" + table))) {
							//node.east_link_is_OK.addRT(ypos + "," + (xpos + 1) + "," + ypos + "," + (a1[ypos].length - 1));
							unreached = "0," + (ypos + 1) + "," + xpos + "," + (a1[xpos].length - 1);
							if (listCheck(list2, unreached)) {
								list2.add(unreached);
							}
							//System.out.println("Writing table: " + ypos + "," + (xpos + 1) + "," + ypos + "," + (a1[ypos].length - 1));
						//}
					}
					if ((a1[xpos][ypos + 1].isVisited == false) && success && this.backtrack(xpos, ypos + 1, destx, desty, list2)) {
						return true;
					} else {
//						if (a1[ypos][xpos + 1].isVisited == true) {
//							if (a1[ypos][xpos + 1].north_link_is_OK.getRT().size() != 0 || a1[ypos][xpos + 1].south_link_is_OK.getRT().size() != 0) {
//								System.out.println("Accessing Visted List");
//								if (listCheck(node.east_link_is_OK.getRT(), compList(a1[ypos][xpos + 1].north_link_is_OK.getRT()))) {
//									System.out.println("Adding " + compList(a1[ypos][xpos + 1].north_link_is_OK.getRT()) + " record to Node: " + "[" + ypos + "][" + xpos + "] East list");
//									node.east_link_is_OK.addRT(compList(a1[ypos][xpos + 1].north_link_is_OK.getRT()));
//								}
//								if (listCheck(node.east_link_is_OK.getRT(), compList(a1[ypos][xpos + 1].south_link_is_OK.getRT()))) {
//									System.out.println("Adding " + compList(a1[ypos][xpos + 1].south_link_is_OK.getRT()) + " record to Node: " + "[" + ypos + "][" + xpos + "] East list");
//									node.east_link_is_OK.addRT(compList(a1[ypos][xpos + 1].south_link_is_OK.getRT()));
//								}
//							}
//						}
//						if (list2.size() != 0) {
...
//							node.east_link_is_OK.addRT(compList(list2));
//							if (a1[ypos][xpos + 1].isVisited == true) {
//								if (listCheck(list2, compList(a1[ypos][xpos + 1].north_link_is_OK.getRT()))) {
//									list2.add(compList(a1[ypos][xpos + 1].north_link_is_OK.getRT()));
//									node.east_link_is_OK.addRT(compList(list2));
//								}
//								if (listCheck(list2, compList(a1[ypos][xpos + 1].east_link_is_OK.getRT()))) {
//									list2.add(compList(a1[ypos][xpos + 1].east_link_is_OK.getRT()));
//									node.east_link_is_OK.addRT(compList(list2));
//								}
//							}
//							if (list2.size() == 2) {
...
//								if (a1[ypos - 1][xpos].isVisited == true) {
//									if (a1[ypos - 1][xpos].east_link_is_OK.getRT().size() != 0) {
//										System.out.println("Accessing Visted List");
//										System.out.println("Adding " + compList(a1[ypos - 1][xpos].east_link_is_OK.getRT()) + " record to Node: " + "[" + ypos + "][" + xpos + "] North list");
//										node.north_link_is_OK.addRT(compList(a1[ypos - 1][xpos].east_link_is_OK.getRT()));
//									}
//								}
								//if (!(list2.isEmpty())) {
									//System.out.println("Accessing List2 Node: " + "[" + ypos + "][" + xpos + "]");
									//String ret = this.compList(list2);
									//node.north_link_is_OK.addRT(ret);
									//if (listCheck(list2, ret)) {
									//	list2.add(ret);
									//}
									//if (list2.size() > 1) {
									//	list2.remove(0);
								...
								System.out.println("Accessing List2 Node: " + "[" + xpos + "][" + ypos + "]");
								if (!(a1[xpos - 1][ypos].north_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
									for (String s1 : a1[xpos - 1][ypos].north_link_is_OK.getRT()) {
										list2.add(compList(list2, s1));
									}
								}
								if (!(a1[xpos - 1][ypos].east_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
									for (String s1 : a1[xpos - 1][ypos].east_link_is_OK.getRT()) {
										list2.add(compList(list2, s1));
									}
								} else {
									String temp = 0 + "," + (ypos + 1) + "," + (this.xsize - 1) + "," + (this.ysize - 1);
									for (int k = 0; k < list2.size(); k++) {
										if (compareArea(list2, temp)) {
											list2.remove(list2.get(k));
										}
									}
								}
								if (!(a1[xpos - 1][ypos].south_link_is_OK.getRT().isEmpty()) && (a1[xpos - 1][ypos].isVisited == true)) {
									for (String s1 : a1[xpos - 1][ypos].south_link_is_OK.getRT()) {
										list2.add(compList(list2, s1));
									}
								}
...
//		Array a1 = new Array(this.size);
//		
//		for (int i = 0; i < a1.size; i++) {
//			if (i == 0) {
//				Node node = new Node(0, a1.randomGen(), 0, a1.randomGen());
//				a1.addArray(node);
//			} else {
//				Node node = new Node(a1.getArray().get(i-1).getEast_link(), a1.randomGen(),  0, a1.randomGen());
//				a1.addArray(node);	
//			}
//		}	
//		return a1;
//	}
	...
	/*public static void main(String[] args) {
		...
		Array a1 = new Array(sizex, sizey);
